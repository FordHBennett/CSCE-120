...
def scale_data(x_data, y_data, factor=1000):
    ...
    linear_map = (magnitudes - smallest) * \
        ((factor - 1)/(largest - smallest)) + 1
    # Logarithmically scale magnitudes
    scaled_magnitudes = np.log(linear_map)
    scale_factor = np.divide(scaled_magnitudes, magnitudes,
                             out=np.zeros_like(magnitudes), where=magnitudes != 0)
