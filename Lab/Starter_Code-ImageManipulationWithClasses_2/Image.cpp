...
Image::Image(const Image& replacement) : image(nullptr), width(0), height(0), filename(replacement.filename){
  clear();
  allocateImage(replacement.getWidth(), replacement.getHeight());
  for (unsigned int row = 0; row<height; row++) {
      for (unsigned int col = 0; col<width; col++) {
        image[col][row] = replacement[col][row];
      }
    }
}

Image& Image::operator= (const Image& replacement) {
  if (this != &replacement) {           // 1. Don't self-assign
    this->clear();                  // 2. Delete old dataObject
    this->allocateImage(replacement.getWidth(), replacement.getHeight());
    for (unsigned int row = 0; row<height; row++) {
      for (unsigned int col = 0; col<width; col++) {
        image[col][row] = replacement[col][row];
      }
